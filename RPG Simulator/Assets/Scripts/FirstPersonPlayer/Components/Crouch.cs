...
public class Crouch : NetworkBehaviour
{
    ...
    public NetworkTransform headToLower;
    [HideInInspector]
    ...
    void Reset()
    {
        ...
        headToLower = movement.GetComponentInChildren<Camera>().gameObject.GetComponent<NetworkTransform>();
    ...
    void LateUpdate()
    {
        if (Input.GetKey(key))
        {
            ...
            if (headToLower)
            {
                ...
                {
                    defaultHeadYLocalPosition = headToLower.transform.localPosition.y;
                }

                // Lower the head.
                headToLower.transform.localPosition = new Vector3(headToLower.transform.localPosition.x, crouchYHeadPosition, headToLower.transform.localPosition.z);
            }
        ...
        else
        {
            if (IsCrouched)
            {
                ...
                if (headToLower)
                {
                    headToLower.transform.localPosition = new Vector3(headToLower.transform.localPosition.x, defaultHeadYLocalPosition.Value, headToLower.transform.localPosition.z);
                }
